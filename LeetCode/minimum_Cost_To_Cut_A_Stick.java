...
public class minimum_Cost_To_Cut_A_Stick {
    public int minCost(int n, int[] cuts) {
        ...
        return tabulation(newCuts, c + 2, dp);
    }

    private int recursion(int[] cuts, int start, int end) {
        if (start >= end)
            return 0;

        int min = 1000000000;
        for (int cut = start + 1; cut < end; cut++) {
            int cost = (cuts[end] - cuts[start]) + recursion(cuts, start, cut)
                    + recursion(cuts, cut, end);
            min = Math.min(min, cost);
        }

        return (min != 1000000000) ? min : 0;
    }

    private int memoization(int[] cuts, int start, int end, int[][] dp) {
        if (start >= end)
            return 0;
        ...
        int min = 1000000000;
        for (int cut = start + 1; cut < end; cut++) {
            int cost = (cuts[end] - cuts[start]) + memoization(cuts, start, cut, dp)
                    + memoization(cuts, cut, end, dp);
            min = Math.min(min, cost);
        }
        
        return dp[start][end] = (min != 1000000000) ? min : 0;
    }

    private int tabulation(int[] cuts, int n, int[][] dp) {
        for (int start = n - 1; start >= 0; start--) {
            for (int end = start + 1; end < n; end++) {
                int min = 1000000000;
                for (int cut = start + 1; cut < end; cut++) {
                    int cost = (cuts[end] - cuts[start]) + dp[start][cut]
                            + dp[cut][end];
                    min = Math.min(min, cost);
                }
                dp[start][end] = (min != 1000000000) ? min : 0;
            }
        }
        return dp[0][n - 1];
    }
